...
class CSVStore(Store):
    ...
    def write(self, stock_code, updated_data):
        ...
        if os.path.exists(csv_file_path):
            ...
            #updated_data_start_date = updated_data['date'].iloc[0]#updated_data[0][-1]
            #old_his = his[his.date < updated_data_start_date]
            #updated_his = pd.DataFrame(updated_data, columns=his.columns)
            #his = updated_his.append(old_his)
            
            updated_data_start_date = his['date'].iloc[-1]
            updated_data_new = updated_data[updated_data.date>updated_data_start_date]
            his = his.append(updated_data_new)
        else:
    ...
    def ts_get_all_stock_codes(self):
        df = ts.get_stock_basics()
        stock_code = list(df.index)
        return stock_code

    @property
    def init_stock_codes(self):
        stock_codes = self.ts_get_all_stock_codes()#easyutils.stock.get_all_stock_codes()
        exists_codes = set()
