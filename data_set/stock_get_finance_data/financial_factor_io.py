  ...
  def fetch_selected_factors(self, factors, dates):
    if not os.path.exists(self.path_factor_io):
      ...
      for factor in factors:
        ...
        factor_value = self.fetch_one_factor(factor, dates)
        pd_factor_values = pd.concat([pd_factor_values, factor_value], axis=1)
      pd_factor_values.to_csv(self.path_factor_io, encoding='ANSI')
    else:
...
if __name__ == '__main__':
  ...
  ffio = financial_factor_io(path=path, path_factor=path_factor, stocks = stocks, dates = dates)
  indexs = [
    #earning capacity
    FID['roe'],\
    FID['roa'],\
