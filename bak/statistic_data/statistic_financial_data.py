...
from finance_load_store import finance_load_store as FLS
...
class financail_factor_statistic:
  ...
  def finance_factor_calc(self,stock_code):
    equlity = self.FLS.fetch_one_finance_factor_in_stock('main','股东权益不含少数股东权益(万元)')
    asset = self.FLS.fetch_one_finance_factor_in_stock('main','总资产(万元)')
    revenue = self.FLS.fetch_one_finance_factor_in_stock('main','主营业务收入(万元)')
    cash = self.FLS.fetch_one_finance_factor_in_stock('main','经营活动产生的现金流量净额(万元)')
    debt = self.FLS.fetch_one_finance_factor_in_stock('main','总负债(万元)')
    
    earning = self.FLS.fetch_one_finance_factor_in_stock('profit','净利润(万元)')
    interest = self.FLS.fetch_one_finance_factor_in_stock('profit','利息支出(万元)')
    tax0 = self.FLS.fetch_one_finance_factor_in_stock('profit','所得税费用(万元)')
    #tax1 = self.FLS.fetch_one_finance_factor_in_stock('profit','营业税金及附加(万元)')
    cost = self.FLS.fetch_one_finance_factor_in_stock('profit','营业总成本(万元)')
    EBIT = earning + interest + tax0
   
    equity = self.FLS.fetch_one_finance_factor_in_stock('loans','所有者权益(或股东权益)合计(万元)')
    debt_total = self.FLS.fetch_one_finance_factor_in_stock('loans','负债合计(万元)')
    intangible_asset = self.FLS.fetch_one_finance_factor_in_stock('loans','无形资产(万元)')
    dev_cost = self.FLS.fetch_one_finance_factor_in_stock('loans','开发支出(万元)')
    goodwell = self.FLS.fetch_one_finance_factor_in_stock('loans','商誉(万元)')
    fix_asset = self.FLS.fetch_one_finance_factor_in_stock('loans','固定资产(万元)')
    noncurrent_asset = intangible_asset + goodwell + fix_asset
    
    
    depreciation = self.FLS.fetch_one_finance_factor_in_stock('cash',' 固定资产折旧、油气资产折耗、生产性物资折旧(万元)')
    amortize0 = self.FLS.fetch_one_finance_factor_in_stock('cash',' 无形资产摊销(万元)')
    amortize1 = self.FLS.fetch_one_finance_factor_in_stock('cash',' 长期待摊费用摊销(万元)')
    excess_cash = self.FLS.fetch_one_finance_factor_in_stock('cash',' 现金的期末余额(万元)')
    #cash_quivalent = self.FLS.fetch_one_finance_factor_in_stock('cash',' 期末现金及现金等价物余额(万元)')
    cash_quivalent = self.FLS.fetch_one_finance_factor_in_stock('cash',' 加:期初现金及现金等价物余额(万元)')
    
    divedends = self.FLS.fetch_one_finance_factor_in_stock('cash',' 分配股利、利润或偿付利息所支付的现金(万元)')
    ...
    pd_data.index = self.FLS.all_finance_one_stock['main'].iloc[0,:].index[1:]
    pd_data = pd_data.sort_index(ascending=True)
    return pd_data #pd.DataFrame([roe roa profit_revenue profit_cost])
  
def main_finance_statistic_process(path):
  ...
  for stock_code in stock_codes:
    ...
    FFC.FLS.load_all_finance_data(stock_code)
    FFC.FLS.load_all_processed_stock_basic_one_stock([stock_code])
    data_processed = FFC.finance_factor_calc(stock_code)
...
if __name__ == '__main__':
  ...
  path = '../../../data/'
  main_finance_statistic_process(path)
